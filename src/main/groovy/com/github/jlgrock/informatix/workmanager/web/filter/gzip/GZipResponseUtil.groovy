...
final class GZipResponseUtil {
    ...
    static final Logger LOG = LoggerFactory.getLogger(GZipResponseUtil.class)
     ...
     */
    static final int EMPTY_GZIPPED_CONTENT_SIZE = 20
    ...
    static boolean shouldBodyBeZero(HttpServletRequest request, int responseStatus) {
        ...
        if (responseStatus == HttpServletResponse.SC_NO_CONTENT) {
            if (LOG.debugEnabled) {
                LOG.debug("{} resulted in a {} response. Removing message body in accordance with RFC2616.",
                        request.requestURL, HttpServletResponse.SC_NO_CONTENT)
            }
            return true
        }
        ...
        if (responseStatus == HttpServletResponse.SC_NOT_MODIFIED) {
            if (LOG.debugEnabled) {
                LOG.debug("{} resulted in a {} response. Removing message body in accordance with RFC2616.",
                        request.requestURL, HttpServletResponse.SC_NOT_MODIFIED)
            }
            return true
        }
        return false
