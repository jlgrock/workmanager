...
public class DatabaseHealthIndicator extends AbstractHealthIndicator {
    ...
    static {
        queries["HSQL Database Engine"] = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.SYSTEM_USERS"
        queries["Oracle"] = "SELECT 'Hello' from DUAL"
        queries["Apache Derby"] = "SELECT 1 FROM SYSIBM.SYSDUMMY1"
        queries["MySQL"] = "SELECT 1"
        queries["PostgreSQL"] = "SELECT 1"
        queries["Microsoft SQL Server"] = "SELECT 1"
    }
    ...
    protected void doHealthCheck(Health.Builder builder) throws Exception {
        ...
        if (StringUtils.hasText(query)) {
            try {
                builder.withDetail("hello", jdbcTemplate.queryForObject(query, Object.class))
            } catch (Exception ex) {
                builder.down(ex)
            }
        }
        ...
        jdbcTemplate.execute(
            new ConnectionCallback() {
                ...
                Object doInConnection(Connection con) throws SQLException, DataAccessException {
                    con.metaData.databaseProductName
                }
            }
        );
    }
